...
import { Slot } from "@radix-ui/react-slot"; // Wait, since it's a zero-dep fallback or we assume they will install radix-ui slot, we can write a clean, native button skeleton that handles standard refs and custom Slot option.
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    const Comp = asChild ? "span" : "button"; // Fallback if slot isn't active
    return (
      <Comp
